...
function KnitServer.CreateTemporaryService( serviceName: string ): ( {} )
	local temporaryService = { _temporaryService = true; }
	KnitServer.Services[ serviceName ] = temporaryService
	return temporaryService
end


function KnitServer.InjectTable( targetTable: {}, tableToInject: {} ): ()
...
function KnitServer.CreateService(serviceDef: ServiceDef): Service
	...
	local service: Service|{} = KnitServer.Services[ serviceDef.Name ] or {}

	-- Remove any temporary data
	service._temporaryService = nil

	-- Inject knit members
	KnitServer.InjectTable( service, {
	...
	-- Inject serviceDef
	KnitServer.InjectTable( service, serviceDef )
...
function KnitServer.GetService(serviceName: string): Service
	...
	local foundService = KnitServer.Services[ serviceName ] or ( (not startedComplete) and KnitServer.CreateTemporaryService(serviceName) )
	assert( foundService, "Could not find service \"" .. serviceName .. "\"" )

	return foundService :: Service|{}
end
...
function KnitServer.Start()
	...
	return Promise.new(function(resolve)
		...
		for _,service in pairs(services) do
			if ( not service._temporaryService ) then
		...
		-- Let user know of any uninitilized temporary services
		local temporaryServiceNames = {}
		for serviceName: string, service: {} in pairs( services ) do
			if ( service._temporaryService ) then
				table.insert( temporaryServiceNames, serviceName )
			end
		end
		if ( #temporaryServiceNames > 0 ) then
			warn( "[KnitServer] Some referenced services were never initialized:", table.concat(temporaryServiceNames, ", ") )
		end
	end)
