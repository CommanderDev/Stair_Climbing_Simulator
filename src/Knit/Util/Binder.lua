...
function Binder.new(tagName, constructor)
	...
	--self._primaryPartName = constructor.PrimaryPartName
...
function Binder:Init()
	...
	for _, inst in pairs(CollectionService:GetTagged(self._tagName)) do
		local conn = bindable.Event:Connect(function()
			self:_waitForStreamedObject(inst, function()
				self:_add(inst)
			end)
		end)
...
function Binder:_hasBasePart(object)
...
function Binder:_waitForStreamedObject(object, func)
	if STREAMING_CLASS_NAMES[object.ClassName] then
		if self:_hasBasePart(object) then
			func()
		else
			local WaitConnection
			WaitConnection = object.ChildAdded:Connect(function()
				if not self:_hasBasePart(object) then return end
				WaitConnection:Disconnect()
	
				func()
			end)
			self._maid:GiveTask(WaitConnection)
		end
	else
		func()
	end
