--| SETTINGS |--
local MAX_HEIGHT = 20; -- MAX HEIGH PLAYER CAN BE FROM GROUND TO BE IN A FIELD.
...
function Field.new(PartTable)
	...
	self.raycastParams.FilterDescendantsInstances = PartTable
...
function Field:CheckPlayer(Player)
	...
	if Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") and Player.Character.Humanoid.Health > 0 then
		...
		local ray = workspace:Raycast(Player.Character.HumanoidRootPart.Position, Vector3.new(0, -MAX_HEIGHT, 0), self.raycastParams)
